...
function New-WPFObject {
    ...
    process {
        ...
        $RawXaml = if ($PSBoundParameters.ContainsKey('Path')) { 
            # ファイルを直接バイトとして読み込み、その後エンコーディングを指定して文字列に変換
        ...
        if ($PSCmdlet.ParameterSetName -in @('PathDynamic', 'HereStringDynamic')) {
            ...
            # XamlReader.Parseを使用してXAMLを解析
            [System.Windows.Markup.XamlReader]::Parse($RawXaml, $ParserContext)
        } else {
            # XamlReader.Parseを使用してXAMLを解析（ParserContextなし）
...
.NOTES
このクラスを継承して、具体なViewModelクラスを作成することができます。
...
.NOTES
こクラスは、WPFアプリケーションのMVVM（Model-View-ViewModel）パターンで
よく使用されます。ViewModelの中でこのクラスのインスタンスを作成し、
XAMLのButtonのCommandプロパティにバインドし使用します。
